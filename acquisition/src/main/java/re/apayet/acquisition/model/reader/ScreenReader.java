...
import re.apayet.acquisition.model.data.ScreenData;

import java.awt.*;

public class ScreenReader extends Reader<ImageData> implements NativeKeyListener, NativeMouseListener {

  private static Logger LOG = LogManager.getLogger(ScreenReader.class.getName());
  ...
  public void addEvent(NativeKeyEvent input, ActionKey action) {
    if (isWork()) {
      ...
      addData(new ScreenData(createScreenCapture(), when, action));
      LOG.debug(action + " : Screen Adding [ " + input.getKeyChar() + " ] - [ When : " + when + " ]");
    }
  }
  ...
  public void addEvent(NativeMouseEvent input, ActionKey action) {
    if (isWork()) {
      Long when = System.currentTimeMillis();
      addData(new ScreenData(createScreenCapture(), when, action));
      LOG.debug(action + " : Screen Adding [ x : " + input.getX() + " ] - [ y : " + input.getY() + " ] - [ When : " + when + " ]");
    }
  }

  private byte[] createScreenCapture() {
    byte[] value = null;
    try {
      value = super.createImage(new Robot().createScreenCapture(new Rectangle(Toolkit.getDefaultToolkit().getScreenSize())));
